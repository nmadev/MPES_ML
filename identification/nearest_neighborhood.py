...
def neighbors(temp_train, temp_test, target_train, target_test):
...
if __name__ == '__main__':
    ...
    for i in [3, 5]:
        ...
        for j in range(0, 4):
            mat_arr = []
            for k in range(0, 3):
                conf_mat = neighbors(temp_training[k][j], temp_testing[k][j], training[k][1], testing[k][1])
        ...
        for j in range(0, 16):
            ...
            for k in range(0, 3):
                conf_mat = neighbors(pair_train[k], pair_test[k], training[k][1], testing[k][1])
                conf_mat = conf_mat.astype('float') / conf_mat.sum(axis=1)[:, np.newaxis]
                mat_arr.append(np.transpose(conf_mat))
            ave_mat = average_mat(mat_arr)
        ...
        for j in range(0, 4):
            ...
            for k in range(0, 3):
                conf_mat = neighbors(trio_train[k], trio_test[k], training[k][1], testing[k][1])
                conf_mat = conf_mat.astype('float') / conf_mat.sum(axis=1)[:, np.newaxis]
                mat_arr.append(np.transpose(conf_mat))
            ave_mat = average_mat(mat_arr)
        ...
        for k in range(0, 3):
            conf_mat = neighbors(quad_train[k], quad_test[k], training[k][1], testing[k][1])
            conf_mat = conf_mat.astype('float') / conf_mat.sum(axis=1)[:, np.newaxis]
            mat_arr.append(np.transpose(conf_mat))
        ave_mat = average_mat(mat_arr)
