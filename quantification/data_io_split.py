...
split_labels = ['CH4/ppm', 'NH3/ppm', 'NatGas_CH4/ppm']
    ...
    for i in range(3):
        for row in data[i]:
            for temp in range(0, 4):
                ...
                temp_arr[temp][i].append(new_set)
...
def format_quantification(df_arr):
    ...
    for label in range(0, 3):
        for index, row in (df_arr[label]).iterrows():
            ...
            i = 0
            if float(row['NatGas_CH4/ppm']) > 0:
                i = 2
            elif float(row['NH3/ppm']) > 0:
                i = 1
            target_vals[i].append(new_target)
            formatted[i].append(new_row)
